...
/**
 * JSONL 的文件系统接缝刻意保持很窄。课程测试可以在不依赖时钟的情况下，
 * 精确控制 append 的开始、失败和部分写入。
 */
...
/**
 * 把外部 unknown 收窄成可持久化的 canonical SessionEntry，并同时返回深副本。
 */
...
/**
 * 空行只是排版，不代表 entry。非空行只有在换行符提交后才会被解析：
 * 最后一段没有换行时，无论内容看起来是否完整，都按未提交尾部忽略。
 */
...
/**
 * 只检查被选 leaf 的祖先链；其他分支可以暂时不完整。id 重复会让指针
 * 失去唯一含义，因此仍在全库范围内拒绝。
 */
...
/**
 * 从选中的 session path 投影 canonical messages。metadata 不进入模型上下文，
 * toolCall、toolResult、isError 和结构化 details 都原样保留。
 */
